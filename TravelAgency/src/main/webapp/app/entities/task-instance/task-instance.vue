<template>
  <div>
    <h2 id="page-heading" data-cy="TaskInstanceHeading">
      <span v-text="$t('travelAgencyApp.taskInstance.home.title')" id="task-instance-heading">Task Instances</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('travelAgencyApp.taskInstance.home.refreshListLabel')">Refresh List</span>
        </button>
      </div>
    </h2>
    <br />
    <div class="alert alert-warning" v-if="!isFetching && taskInstances && taskInstances.length === 0">
      <span v-text="$t('travelAgencyApp.taskInstance.home.notFound')">No taskInstances found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="taskInstances">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('travelAgencyApp.taskInstance.taskId')">Task Id</span></th>
            <th scope="row"><span v-text="$t('travelAgencyApp.taskInstance.name')">Name</span></th>
            <th scope="row"><span v-text="$t('travelAgencyApp.taskInstance.status')">Status</span></th>
            <th scope="row"><span v-text="$t('travelAgencyApp.taskInstance.createDate')">Create Date</span></th>
            <th scope="row"><span v-text="$t('travelAgencyApp.taskInstance.createTime')">Create Time</span></th>
            <th scope="row"><span v-text="$t('travelAgencyApp.taskInstance.dueDate')">Due Date</span></th>
            <th scope="row"><span v-text="$t('travelAgencyApp.taskInstance.startTime')">Start Time</span></th>
            <th scope="row"><span v-text="$t('travelAgencyApp.taskInstance.endTime')">End Time</span></th>
            <th scope="row"><span v-text="$t('travelAgencyApp.taskInstance.owner')">Owner</span></th>
            <th scope="row"><span v-text="$t('travelAgencyApp.taskInstance.assignee')">Assignee</span></th>
            <th scope="row"><span v-text="$t('travelAgencyApp.taskInstance.executionId')">Execution Id</span></th>
            <th scope="row"><span v-text="$t('travelAgencyApp.taskInstance.taskDefinitionKey')">Task Definition Key</span></th>
            <th scope="row"><span v-text="$t('travelAgencyApp.taskInstance.suspended')">Suspended</span></th>
            <th scope="row"><span v-text="$t('travelAgencyApp.taskInstance.priority')">Priority</span></th>
            <th scope="row"><span v-text="$t('travelAgencyApp.taskInstance.processDefinition')">Process Definition</span></th>
            <th scope="row"><span v-text="$t('travelAgencyApp.taskInstance.processInstance')">Process Instance</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
